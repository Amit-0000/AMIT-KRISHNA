...
class ProfilePage(BasePage):
    ...
    def blur_display_name(self) -> None:
        # Blurs the focused field without submitting a real update -- the
        # zod resolver here has no explicit `mode`, so react-hook-form's
        # default (validate on submit) applies; the profile save button is
        # disabled until the form is dirty anyway (see is_save_disabled),
        # so this is only useful for tests that want the field un-focused.
...
class AppearancePage(BasePage):
    ...
    def theme_option(self, label: str):
        ...
        # Appearance.tsx).
        return self.find(By.XPATH, f'//div[@role="radiogroup"][@aria-label="Theme"]//button[.//p[normalize-space()="{label}"]]')

    def select_theme(self, label: str) -> None:
        self.tap(self.theme_option(label))
    ...
    def set_nav_compact(self) -> None:
        self.tap(self.find_clickable(*self.NAV_COMPACT_BUTTON))

    def set_nav_expanded(self) -> None:
        self.tap(self.find_clickable(*self.NAV_EXPANDED_BUTTON))
