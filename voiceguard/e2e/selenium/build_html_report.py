"""Builds e2e/results/execution-report.html from e2e/results/selenium_summary.json
(parse_results.py's output). Pure stdlib string templating, no Jinja/pandas —
adapted from voiceguard/e2e/appium/build_html_report.py (same shape/palette,
browser info instead of Android/emulator info) so the QA suite's reports
read as one family regardless of which suite produced them.
"""
...
<html><head><meta charset="utf-8"><title>VoiceGuard — Selenium Web E2E Report</title>
...
def priority_summary_rows(test_rows: list[dict]) -> str:
    counts: dict[str, dict[str, int]] = {}
    for t in test_rows:
        b = counts.setdefault(t["priority"], {"total": 0, "passed": 0})
        b["total"] += 1
        if t["status"] == "passed":
            b["passed"] += 1
    rows = []
    for p in ("critical", "high", "medium", "low", "unspecified"):
        if p not in counts:
            continue
        b = counts[p]
        pct = round(100 * b["passed"] / b["total"], 1) if b["total"] else 0
        color = PRIORITY_COLORS[p]
        rows.append(
            f'<tr><td style="color:{color};font-weight:600;text-transform:capitalize">{p}</td>'
            f'<td>{b["total"]}</td><td>{b["passed"]}</td><td>{pct}%</td></tr>'
        )
    return "\n".join(rows)
...
<title>VoiceGuard — Selenium Web E2E Execution Report</title>
<style>
...
<body>
<div class="wrap">
  <h1>VoiceGuard — Selenium Web E2E Execution Report</h1>
  <div class="subtitle">Generated {esc(summary["generated_at"])} · {esc(env.get("BROWSER",""))} ·
    {esc(env.get("WINDOW_SIZE",""))} · {esc(env.get("SELENIUM_BASE_URL",""))}</div>
  ...
  <div class="card">
    <h2>By priority</h2>
    <table>
      <tr><th>Priority</th><th>Total</th><th>Passed</th><th>Pass %</th></tr>
      {priority_summary_rows(summary["test_rows"])}
    </table>
  </div>
